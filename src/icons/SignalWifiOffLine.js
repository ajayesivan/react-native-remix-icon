import * as React from "react";
import Svg, { Path } from "react-native-svg";

function SvgSignalWifiOffLine(props) {
  return (
    <Svg
      viewBox="0 0 24 24"
      fill="currentColor"
      xmlns="http://www.w3.org/2000/svg"
      {...props}
    >
      <Path d="M2.808 1.393l17.678 17.678-1.414 1.414-3.683-3.682L12 21 .69 6.997c.914-.74 1.902-1.391 2.951-1.942L1.394 2.808l1.414-1.415zm.772 5.999L12 17.817l1.967-2.437-8.835-8.836c-.532.254-1.05.536-1.552.848zM12 3c4.285 0 8.22 1.497 11.31 3.996l-5.406 6.693-1.422-1.422 3.938-4.876A15.922 15.922 0 0012 5c-.872 0-1.734.07-2.579.207L7.725 3.51C9.095 3.177 10.527 3 12.001 3z" />
    </Svg>
  );
}

export default SvgSignalWifiOffLine;
